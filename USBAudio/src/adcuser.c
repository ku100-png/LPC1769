    ...
    if (SetupPacket.wValue.WB.L == 0) {
      ...
      switch (SetupPacket.wValue.WB.H) {
        ...
        case AUDIO_VOLUME_CONTROL:
          switch (SetupPacket.bRequest) {
            case AUDIO_REQUEST_GET_CUR:
                *((__attribute__((packed)) uint16_t *)EP0Buf) = VolCur;
//                *((__packed uint16_t *)EP0Buf) = VolCur;
              return (TRUE);
            case AUDIO_REQUEST_GET_MIN:
              *((__attribute__((packed)) uint16_t *)EP0Buf) = VolMin;
//              *((__packed uint16_t *)EP0Buf) = VolMin;
              return (TRUE);
            case AUDIO_REQUEST_GET_MAX:
              *((__attribute__((packed)) uint16_t *)EP0Buf) = VolMax;
//              *((__packed uint16_t *)EP0Buf) = VolMax;
              return (TRUE);
            case AUDIO_REQUEST_GET_RES:
              *((__attribute__((packed)) uint16_t *)EP0Buf) = VolRes;
    ...
    if (SetupPacket.wValue.WB.L == 0) {
      ...
      switch (SetupPacket.wValue.WB.H) {
        ...
        case AUDIO_VOLUME_CONTROL:
          switch (SetupPacket.bRequest) {
            case AUDIO_REQUEST_SET_CUR:
            	VolCur = *((__attribute__((packed)) uint16_t *)EP0Buf);
